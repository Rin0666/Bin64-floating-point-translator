...
public class MainController implements ActionListener
{
    ...
    private static String getFixedFloat(String norm){
        float fnorm=Float.parseFloat(norm);
        int ctr = 0;
        if (fnorm == 0) {return String.valueOf("0");}
        while(fnorm >=10 || fnorm <1){
            System.out.printf("exp %d = %f\n", ctr, fnorm);
            if(fnorm >= 10){
                ctr ++;
                fnorm = fnorm /10;
            }
